...
bool SMDirectoryEntry_Read(struct SMDirectoryEntry* self, uint32_t size, uint8_t* buffer)
{
    ...
    for (i=0; i<sectors_required; i++)
    {
        ...
        if(FatVolume_ReadSector(
            ...
            /*FatBPB_GetFirstDataSector(&self->Volume->FatVolume.BiosParameterBlock)*/ 
...
bool SMDirectoryEntry_Write(struct SMDirectoryEntry* self, uint32_t size, uint8_t* buffer)
{
    return false;
}