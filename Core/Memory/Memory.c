...
void* Memory_Allocate(struct Memory* self, uint32_t size)
{
	...
	uint32_t freeBlock = Memory_FindFreeBlock(self, size);
	...
	if (freeBlock == self->HeapEndAddress)
	{
		...
		Memory_MoveHeapEnd(self,totalRequestedSize);
	}
...
void Memory_Free(struct Memory* self, void* addr)
{
	...
	if ((uint32_t)self->HeapEndAddress == (uint32_t)header+sizeof(struct Memory_BlockHeader)+header->mSize)
	{
		...
		Memory_MoveHeapEnd(self, -totalSize);
	}
}
