...
void ATA_Constructor()
{
    ...
    if (ata_device)
    {
        ...
        if ((IO_ReadPortLong(0xCFC) & 0xFF) == 0xFE) 
        {
            printf("ATA: Device needs IRQ Assignment (NOT IMPLEMENTED)\n");
        } 
        ...
        */
        ...
        ATA_IDEInit(
            ...
            b0,b1,b2,b3,0x00//ata_device->mBAR4
        );
    }
    else
    {
...
) 
{
   ...
   for (i = 0; i < 2; i++)
   {
      for (j = 0; j < 2; j++) 
      {
         ...
         // (III) Polling:
         printf("ATA: Polling %d,%d\n",i,j);
         if (!(ATA_IDERead(channels, i, ATA_REG_STATUS))) 
         {
            printf("ATA: Polling %d,%d - No Device\n",i,j);
            continue; // If Status = 0, No Device.
         }
        printf("ATA: After Polling\n");
         while(1) 
         {
            status = ATA_IDERead(channels, i, ATA_REG_STATUS);
            if ( (status & ATA_SR_ERR)) 
            {
                printf("ATA: Polling %d,%d Error status\n",i,j);
                err = 1; 
                break;
            } // If Err, Device is not ATA.
            if (!(status & ATA_SR_BSY) && (status & ATA_SR_DRQ)) 
            {
                printf("ATA: Polling %d,%d Not ATA\n",i,j);
         ...
         printf("ATA: Probing for ATAPI %d,%d\n",i,j);
         if (err) 
         {
   ...
   for (i = 0; i < 4; i++)
   {
      ...
      {
          printf("ATA: No summary for device %d\n",i);
      }
    }
}
...
unsigned char ATA_IDERead(ATA_Channel* channels, unsigned char channel, unsigned char reg) 
{
    ...
    if (reg < 0x08) 
    {
        result = IO_ReadPort(channels[channel].base  + reg - 0x00);
    }
    else if (reg < 0x0C) 
    {
        result = IO_ReadPort(channels[channel].base  + reg - 0x06);
    }
    else if (reg < 0x0E) 
    {
        result = IO_ReadPort(channels[channel].ctrl  + reg - 0x0A);
    }
    else if (reg < 0x16) 
    {
        result = IO_ReadPort(channels[channel].bmide + reg - 0x0E);
    }
...
void ATA_IDEWrite(ATA_Channel* channels, unsigned char channel, unsigned char reg, unsigned char data) 
{
    ...
    if (reg < 0x08) 
    {
        IO_WritePort(data, channels[channel].base  + reg - 0x00);
    }
    else if (reg < 0x0C) 
    {
        IO_WritePort(data, channels[channel].base  + reg - 0x06);
    }
    else if (reg < 0x0E) 
    {
        IO_WritePort(data, channels[channel].ctrl  + reg - 0x0A);
    }
    else if (reg < 0x16) 
    {
        IO_WritePort(data, channels[channel].bmide + reg - 0x0E);
    }
...
void ATA_IDEReadBuffer(ATA_Channel* channels, unsigned char channel, unsigned char reg, unsigned int buffer, unsigned int quads) 
{
   ...
   if   (reg < 0x08) 
   {
       IO_ReadPortLongString(channels[channel].base  + reg - 0x00, (void*)buffer, quads);
   }
   else if   (reg < 0x0C) 
   {
       IO_ReadPortLongString(channels[channel].base  + reg - 0x06, (void*)buffer, quads);
   }
   else if   (reg < 0x0E) 
   {
       IO_ReadPortLongString(channels[channel].ctrl  + reg - 0x0A, (void*)buffer, quads);
   }
   else if   (reg < 0x16) 
   {
       IO_ReadPortLongString(channels[channel].bmide + reg - 0x0E, (void*)buffer, quads);
   }
